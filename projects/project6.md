---
id: project6
github: https://world970511.github.io/
title:
  ko: 개인 포트폴리오 사이트 겸 블로그
  en: Personal Portfolio Website and Blog
tech:
  ko: HTML, CSS, JavaScript, Github Actions
  en: HTML, CSS, JavaScript, Github Actions
---

## 💡 Motivation

**[ko]**
이전에 사용하던 블로그를 이전할 겸 포트폴리오와 통합하였습니다.

**[en]**
Migrated from the previous blog to integrate with the portfolio.

## ✨ Features

**[ko]**
- 순수 JS,HTML,CSS 기반의 개인 포트폴리오 사이트
- GitHub Action을 활용한 CI/CD

**[en]**
- Personal portfolio website built with pure JS, HTML, and CSS
- CI/CD with GitHub Actions

## 📊 Results

**[ko]**
- 개인 포트폴리오 사이트와 블로그를 통합하여 사용자 경험을 개선하였습니다.
- GitHub Action을 활용한 CI/CD를 구현하여 빠른 배포를 가능하게 하였습니다.

**[en]**
- Integrated personal portfolio website and blog to improve user experience
- Implemented CI/CD with GitHub Actions for faster deployment
