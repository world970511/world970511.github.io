...
<summary>@app.task vs @shared_task</summary>
    ** 인스턴스 의존성 **  
    @app.task: Celery 앱 인스턴스(app)를 명시적으로 가져와야 함  
    @shared_task: Celery 인스턴스 없이 정의 가능, 독립적으로 작동  

    ** 사용 환경 **  
    @app.task: 일반적인 Python 프로젝트에서 사용  
    @shared_task: Django 프로젝트나 재사용 가능한 앱에서 사용 적합  

    ** Django 사용 시 추천 여부 **  
    @app.task: Django 환경에서는 비추천  
    @shared_task: Django 통합 시 강력히 권장  
...
<summary>@delay() vs apply_async()</summary>
    ** 사용 목적 **  
    @delay(): 빠르고 단순한 비동기 호출용  
    apply_async(): 고급 설정 및 세밀한 제어가 필요한 경우에 사용  

    ** 인수 전달 방식 **  
    @delay(): 일반 함수 호출처럼 인수를 직접 전달 (task.delay(a, b))  
    apply_async(): args=(), kwargs={} 형태로 명시적으로 전달  

    ** Celery 옵션 사용 여부 **  
    @delay(): Celery의 추가 옵션(queue, countdown, eta 등) 사용 불가  
    apply_async(): 다양한 Celery 실행 옵션 사용 가능  
</details>
