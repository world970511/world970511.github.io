...
그러다보니 관심사가 조금... 클라우드보다는 온프레미스 쪽에 맞춰져 있는데 솔직히 저 환경에서 데이터 관리하면 다들 그럴 거라고 생각한다 관리하기는 힘든데 그렇다고 이것저것 마음대로 도입하기에는 어려워서.....;;;  
진짜 업무하면서 이 환경에서 데이터 관리를 진짜 어떻게 해야 하는걸까? 싶었고 진짜 고민도 많이 하고 이것저것 손 대본 것 같다.  
뭐 여튼 이런 배경으로 AI가 자동으로 데이터 카탈로그도 짜주고 마트도 구성해주고 데이터관리도 되고 중복 데이터 관리도 되고 다 해주면 좋겠다! 라는 생각으로 이 프로젝트를 시작하게 되었다.  
...
- 관리자/사용자 권한을 분리해야 한다.  
영업팀이나 모델 개발 팀 쪽에서 데이터 마음대로 삭제했다가 문제가 생기는 걸 많이 봐서... HITL(Human in the loop)부분을 조금 신경을 썼다.(에이전트가 마트 구축만 가능하고 삭제는 허가를 받도록 한다든가)  
...
## 🔚결론
  ...
  경량 모델 + cpu 로 돌렸더니 특정 테이블 컬럼 내용만 출력해달랬더니 컬럼이란 컬럼이 없습니다! 라고 에러 띄워서 당황한 적도 있다.
