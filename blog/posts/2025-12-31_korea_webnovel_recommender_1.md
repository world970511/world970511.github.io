...
이런 식으로 추천이 진행되고, 개인적인 이해로는 기존의 콘텐츠 기반 필터링에 백터 임베딩을 결합한 구조로 보고 있다. 
뭔 차이냐 라고 한다면...  
...
```
    "마법사 소년의 모험" → [0.23, -0.45, 0.12, ...] 벡터 변환
                     → 벡터 공간에서 가까운 책 검색
```
그러니까 이전에는 키워드(=단어)간 유사도(코사인 유사도든 뭐든간에)  점수를 기반으로 추천을 해줬고, 지금 활용하려는 건 비슷하긴 하지만 텍스트를 벡터로 변환한 후 백터 공간에서의 거리를 기반으로 추천을 한다는 이야기다.  
